<script setup lang="ts">
  ...
  const headers: TableHeader[] = [
    ...
    { key: 'title', label: 'article title', width: '30%' },
    { key: 'subjects', label: 'subject', width: '15%' },
  ...
  // const deleteArrow = (e_c_branch: TitleInterface<number>) => {
  ...
  const deleteDialogTitle = ref('Are you sure you want to remove this Article?');
  const deleteDialogMessage = ref(
    'Deleting this Article will remove all related data. This action is irreversible, and the Article must be created again if needed.',
  );
</script>
            ...
            <template #cell-subjects="{ item }">
              <div class="subject-cell">
                <div class="parent-subject-curriculum">
                  <span
                    v-for="(subject, index) in getSubjectPath(item)"
                    :key="index"
                    class="subject-curriculum"
                  >
                    <p>{{ subject }}</p>
                    <Articlearrow
                      v-if="index < getSubjectPath(item).length - 1"
                      class="arrow-icon"
                    />
                  </span>
                </div>
                ...
                <span class="subject-cycle">
                  <span class="subject-lang">{{ item.subjects?.title }}</span>
                </span>
              </div>
            </template>
