...
<template>
  ...
  <div class="true-false-answers-time-line-container">
    <div v-for="(item, index) in Answers" :key="index" class="timeline-item"
        ...
        <div class="timeline-form-content">
          <div class="field-group">
            ...
            <div class="input-wrap">
              <input :id="`answer-${index}`" v-model="item.answer" type="text" :placeholder="$t('add_your_answer')"
            ...
            <div class="files-input" :class="{ haveImage: item.image }">
              <HandleFilesUpload :label="``" accept="image/*" :multiple="true" :index="index + 5"
                :file="item.image?.map((img) => img.file).filter(Boolean) as string[]" :have-content="true"
                :class="`image-input`" @change="(files) => setImage(files, index)">
                <template #content>
                  <UploadFileIcon />
                </template>
              </HandleFilesUpload>
          ...
          <div class="is-correct-section">
            ...
            <Checkbox :binary="true" :model-value="item.is_right_answer" :input-id="`is-correct-${index}`"
              name="is-correct" :value="item.is_right_answer" @change="setCorrect(index)" />
          </div>
        </div>
      </div>
    </div>
