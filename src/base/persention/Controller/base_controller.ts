...
import type BaseRepository from '@/base/Domain/Repositories/base_repository';
import type { ApiCallOptions } from '@/base/Data/ApiService/base_api_service';
...
export default abstract class BaseController<T, TList = T[]> {
    ...
    async fetchList(params?: Params, options?: ApiCallOptions): Promise<DataState<TList>> {
        ...
        try {
            const result = await this.repository.index(params, options);
    ...
    protected handleItemResponse(_result: DataState<T>, successMessage?: string): void {
        if (_result instanceof DataSuccess && this.config.showSuccessDialog && successMessage) {
